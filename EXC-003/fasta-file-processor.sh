...
if [ "$(head -c 1 $1)" == ">" ]; then 
	...
	short=$(cat genes-one-line.fna | grep -v '>' genes-one-line.fna | awk '{print length}' | sort -n | head -n 1)

	## average length of sequences
	aveg=$(($total_length_sq/$num_contigs))

	## GC Content (%)
	GC_amount=$(cat genes-one-line.fna | grep -v '>' genes-one-line.fna | awk '{gc_count += gsub(/[GgCc]/, "", $1)} END {print gc_count}')
